...
# тёмными объектами (чёрный текст) даёт полупрозрачную серую дымку по краям.
# Более высокий порог убирает дымку/физер (края чёткие), не съедая объект.
# В резком режиме можно ещё выше (нет апскейл-мыла) -> кромка совсем поджата;
# для печати (апскейл 8K) порог ниже, иначе увеличенное мыло съедает тонкие буквы.
CLIP_BLACK_BLUE = 0.20        # режим печати (SHARP_MODE выкл)
CLIP_BLACK_BLUE_SHARP = 0.36  # резкий режим (SHARP_MODE вкл): кромка без физера
...
# РЕЗКИЙ РЕЖИМ: не увеличивать и не сглаживать -> буквы/линии ровно как в исходнике
# (у присланных картинок край уже сглажен, лишнее сглаживание только мылит).
# Выкл -> прежнее поведение: апскейл до 8K + сглаживание (гладкий край для печати).
SHARP_MODE = True
...
def process(pil_rgb, override_bg=None, preview=False):
    ...
    # Резкий режим -> без увеличения RGB (иначе LANCZOS мылит мелкий чёткий исходник).
    oscale = 1.0 if SHARP_MODE else max(1.0, min(OUTPUT_MAX_SCALE, omax / max(w0, h0)))
    if oscale > 1.001:
    ...
    ss = 1
    if not SHARP_MODE and ANTIALIAS and AA_SS > 1:     # в резком режиме маску не супер-сэмплим
    ...
    choke = 0 if SHARP_MODE else int(round(CONTOUR_CHOKE_SRC * oscale))
    if choke > 0:
        aimg = _fast_erode(aimg, choke)
    feather = 0.0 if SHARP_MODE else ALPHA_FEATHER
    if feather > 0:                       # добить остаточную «лесенку» (только alpha)
        aimg = aimg.filter(ImageFilter.GaussianBlur(radius=feather))
...
def _screen_matte(R, G, B, S, key):
    ...
    m = 1.0 - screen
    if key == 2:
        cb = CLIP_BLACK_BLUE_SHARP if SHARP_MODE else CLIP_BLACK_BLUE  # синий: поджать кромку
    else:
        cb = CLIP_BLACK
...
class App:
    ...
    def _build(self):
        ...
        self.sharp_var = tk.BooleanVar(value=SHARP_MODE)
        ttk.Checkbutton(left, text="Резкий режим (чётко, как в исходнике)",
                        variable=self.sharp_var, command=self._toggle_sharp).pack(anchor="w")
        ttk.Label(left, text="выкл — апскейл до 8K + сглаживание (для печати)",
